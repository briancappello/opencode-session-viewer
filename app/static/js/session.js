...
function updateViz(index) {
    ...
    document.getElementById("progressLabel").textContent = `Message ${index + 1} / ${SESSION_DATA.messages.length}`;
...
function renderSparkline() {
    ...
    const points = tokenData.map((t, i) => {
        const x =
            padding.left + (i / (tokenData.length - 1 || 1)) * chartWidth;
        const y =
            padding.top + chartHeight - (t.cache / maxCache) * chartHeight;
        return { x, y };
    });
    ...
    if (el) {
        ...
        document.querySelectorAll(".message-item").forEach((item) => {
            item.classList.toggle(
                "active",
                parseInt(item.dataset.index) === idx
            );
        });
...
function renderPart(part) {
    ...
    if (part.type === "tool") {
        ...
        const isSubtask =
            part.tool === "task" && part.state?.input?.subagent_type;
        if (isSubtask) return "";
        ...
        const skipHighlight = noHighlightTools.some((t) =>
            toolName.includes(t)
        );
